...
import ReporteMarcacionesModel from "../model/reportes.model.js";
    ...
    try {
        ...
        if (!usuario) {
            return res.status(404).json({
                success: false,
                message: "No se encontrÃƒÂ³ un trabajador con ese RUT",
                encontrado: false
            });
        }

        // Verificar si el trabajador ya estÃƒÂ¡ asociado a la empresa del usuario que hace la peticiÃƒÂ³n
        ...
        // Obtener preferencias de compensaciÃƒÂ³n si existen
...
const createTrabajador = async (req, res) => {
    try {
        ...
        if (existingUserByRut) {
            ...
            // Verificar si ya estÃƒÂ¡ asociado a esta empresa
            ...
            if (existingUserEmpresa) {
                return res.status(400).json({
                    success: false,
                    message: "Este trabajador ya estÃƒÂ¡ asociado a su empresa"
                });
            }
            
            // NUEVO: Crear solicitud de invitaciÃƒÂ³n en lugar de asociar directamente
            try {
                const token = TokenService.generarTokenAceptacion();
                const fechaExpiracion = new Date();
                fechaExpiracion.setDate(fechaExpiracion.getDate() + 7); // 7 dÃƒÂ­as de validez
                ...
                // Enviar correo de invitaciÃƒÂ³n
                ...
                console.log(`Ã¢Å“â€¦ InvitaciÃƒÂ³n enviada a ${existingUserByRut.email} para unirse a ${empresa.emp_nombre}`);
                
                return res.status(200).json({
                    success: true,
                    invitacionEnviada: true,
                    message: `Se ha enviado una invitaciÃƒÂ³n al correo ${existingUserByRut.email}. El trabajador debe aceptarla para unirse a la empresa.`
                });
            } catch (invitacionError) {
                console.error('Ã¢ÂÅ’ Error al enviar invitaciÃƒÂ³n:', invitacionError);
                return res.status(500).json({
                    success: false,
                    message: 'Error al procesar la invitaciÃƒÂ³n',
            ...
            // NUEVO: Crear solicitud de invitaciÃƒÂ³n igual que cuando el usuario ya existe
            try {
                const token = TokenService.generarTokenAceptacion();
                const fechaExpiracion = new Date();
                fechaExpiracion.setDate(fechaExpiracion.getDate() + 7); // 7 dÃƒÂ­as de validez
                ...
                console.log(`Ã¢Å“â€¦ InvitaciÃƒÂ³n enviada a ${newUser.email} para unirse a ${empresa.emp_nombre}`);
                
                return res.status(201).json({
                    success: true,
                    invitacionEnviada: true,
                    message: `Se ha enviado una invitaciÃƒÂ³n al correo ${newUser.email}. El trabajador debe aceptarla para unirse a la empresa.`
                });
            } catch (invitacionError) {
                console.error('Ã¢ÂÅ’ Error al enviar invitaciÃƒÂ³n:', invitacionError);
                return res.status(500).json({
        ...
        // NOTA: Este cÃƒÂ³digo de asociaciÃƒÂ³n directa ya no se ejecuta porque ambos caminos 
        // (usuario existe/no existe) terminan con return en la secciÃƒÂ³n de invitaciÃƒÂ³n
        ...
        try {
            const UsuariosRolesAsignadosModel = (await import('../model/usuarios-roles-asignados.model.js')).default;
            const RolesSistemaModel = (await import('../model/roles-sistema.model.js')).default;
            ...
            if (rolSistema) {
                ...
                console.log(`Ã¢Å“â€¦ Rol '${userData.rol}' asignado al usuario en usuarios_roles_asignados`);
            } else {
                console.warn(`Ã¢Å¡Â Ã¯Â¸Â No se encontrÃƒÂ³ el rol '${userData.rol}' en roles_sistema`);
            }
        } catch (roleError) {
            console.error('Ã¢ÂÅ’ Error al asignar rol en usuarios_roles_asignados:', roleError);
            // No bloqueamos la creaciÃƒÂ³n del trabajador por este error
        }
        ...
        if (req.user && req.user.id) {
            ...
            console.warn('Ã¢Å¡Â Ã¯Â¸Â No se pudo registrar creaciÃƒÂ³n de trabajador en auditorÃƒÂ­a (UserEmpresaController):', {
                hasReqUser: !!req.user,
                userId: req.user?.id,
        ...
        // Crear preferencia de compensaciÃƒÂ³n si se proporciona
        if (userData.preferenciasCompensacion) {
            try {
                ...
                // Validar que el tipo de compensaciÃƒÂ³n sea vÃƒÂ¡lido
                const tiposValidos = ['PAGO', 'DESCANSO', 'mixto'];
                if (!tiposValidos.includes(preferenciaData.tipo_compensacion)) {
                    console.warn('Ã¢Å¡Â Ã¯Â¸Â Tipo de compensaciÃƒÂ³n invÃƒÂ¡lido:', preferenciaData.tipo_compensacion);
                } else {
                    const id_preferencia = await PreferenciasCompensacionModel.crear(preferenciaData);
                    console.log('Ã¢Å“â€¦ Preferencia de compensaciÃƒÂ³n creada:', id_preferencia);
                }
            } catch (prefError) {
                console.error('Ã¢Å¡Â Ã¯Â¸Â Error al crear preferencia de compensaciÃƒÂ³n:', prefError);
                // No bloqueamos la creaciÃƒÂ³n del trabajador si hay error en la preferencia
            }
        }
...
const createTurno = async (req, res) => {
    try {
        ...
        console.log('AsignaciÃƒÂ³n de turno recibida:', asignacionData);
        ...
        // Obtener informaciÃƒÂ³n del tipo de turno
        ...
        // Obtener los dÃƒÂ­as laborables del tipo de turno
        ...
        // Si hay turnos en el rango, verificar conflictos por dÃƒÂ­a
        if (turnosEnRango.length > 0) {
            const diasConflicto = [];

            for (const diaLabor of diasLaborables) {
                if (diaLabor.trabaja) {
                    // Verificar si algÃƒÂºn turno existente tambiÃƒÂ©n trabaja este dÃƒÂ­a
                    for (const turnoExistente of turnosEnRango) {
                        ...
                        if (conflicto) {
                            ...
                            break; // Ya encontramos un conflicto para este dÃƒÂ­a
                        }
                    }
                }
            }
            ...
            if (diasConflicto.length > 0) {
                return res.status(409).json({
                    success: false,
                    message: "El trabajador ya tiene turnos asignados en los siguientes dÃƒÂ­as",
                    conflictos: diasConflicto
                });
            }
        }

        // Crear la asignaciÃƒÂ³n de turno
        const asignacionId = await AsignacionTurnosModel.create(asignacionData);

        // Registrar la asignaciÃƒÂ³n de turno en auditorÃƒÂ­a
        if (req.user && req.user.id) {
            try {
                ...
                console.error('Ã¢Å¡Â Ã¯Â¸Â Error al registrar asignaciÃƒÂ³n de turno en auditorÃƒÂ­a:', auditError);
            }
        }
...
// Eliminar turno por ID - permite borrar turnos asignados especÃƒÂ­ficos
const deleteTurno = async (req, res) => {
    try {
        const { id } = req.params;

        // Validar que el ID del turno sea vÃƒÂ¡lido
        if (!id || isNaN(id)) {
            return res.status(400).json({
                success: false,
                message: "ID de turno invÃƒÂ¡lido"
            });
        }

        // Verificar que la asignaciÃƒÂ³n existe antes de eliminarla
        const asignacionExistente = await AsignacionTurnosModel.getById(id);
        if (!asignacionExistente) {
            return res.status(404).json({
                success: false,
                message: "AsignaciÃƒÂ³n de turno no encontrada"
            });
        }

        // Obtener informaciÃƒÂ³n del trabajador y tipo de turno para auditorÃƒÂ­a
        ...
        // Eliminar la asignaciÃƒÂ³n de turno
        const resultado = await AsignacionTurnosModel.delete(id);

        if (resultado === 0) {
            return res.status(404).json({
                success: false,
                message: "No se pudo eliminar la asignaciÃƒÂ³n de turno"
            });
        }

        // Registrar la eliminaciÃƒÂ³n en auditorÃƒÂ­a
        if (req.user && req.user.id) {
            try {
                ...
                await AuditoriaModel.registrarCambio({
                    ...
                    tabla_afectada: 'asignacion_turnos',
                    registro_id: id,
                    descripcion: `AsignaciÃƒÂ³n de turno eliminada: ${nombreTrabajador} - ${nombreTipoTurno}`,
                ...
                console.error('Ã¢Å¡Â Ã¯Â¸Â Error al registrar eliminaciÃƒÂ³n de turno en auditorÃƒÂ­a:', auditError);
            }
        }

        res.status(200).json({
            success: true,
            message: "AsignaciÃƒÂ³n de turno eliminada exitosamente"
        });
    } catch (error) {
...
const updateTurno = async (req, res) => {
    try {
        ...
        // Verificar que la asignaciÃƒÂ³n existe antes de modificarla
        const asignacionExistente = await AsignacionTurnosModel.getById(id);
        if (!asignacionExistente) {
            return res.status(404).json({
                success: false,
        ...
        // Obtener informaciÃƒÂ³n del nuevo tipo de turno
        ...
        // Obtener informaciÃƒÂ³n del trabajador usando usuario_empresa_id
        ...
        // validar si se estÃƒÂ¡ intentando asignar el mismo tipo de turno
        ...
        // Registrar la modificaciÃƒÂ³n en auditorÃƒÂ­a
        if (req.user && req.user.id) {
            try {
                const nombreTrabajador = trabajador ? `${trabajador.nombre} ${trabajador.apellido_pat || ''}`.trim() : 'Trabajador desconocido';
                ...
                console.log('Ã¢Å“â€¦ ModificaciÃƒÂ³n de turno registrada en auditorÃƒÂ­a');
            } catch (auditError) {
                console.error('Ã¢Å¡Â Ã¯Â¸Â Error al registrar modificaciÃƒÂ³n de turno en auditorÃƒÂ­a:', auditError);
            }
        }
...
// Guardar configuraciÃƒÂ³n del sistema empresarial - permite personalizar ajustes
const guardarConfiguracion = async (req, res) => {
    try {
        ...
        // Por ahora simularemos el guardado - aquÃƒÂ­ se implementarÃƒÂ­a el guardado real en BD


        // Registrar el cambio en auditorÃƒÂ­a - permite seguimiento de configuraciones
        if (USR_PETICION.id) {
            try {
                await AuditoriaModel.registrarCambio({
                    ...
                    descripcion: `ConfiguraciÃƒÂ³n del sistema actualizada - Empresa: ${empresa.emp_nombre || 'Sin nombre'}`,
                    datos_anteriores: null, // AquÃƒÂ­ se podrÃƒÂ­a obtener la configuraciÃƒÂ³n anterior
                    datos_nuevos: JSON.stringify(configuracionData),
                    ip_address: req.ip || req.connection.remoteAddress
                });

            } catch (auditError) {
                console.error('Ã¢Å¡Â Ã¯Â¸Â Error al registrar cambio de configuraciÃƒÂ³n en auditorÃƒÂ­a:', auditError);
            }
        }

        res.status(200).json({
            success: true,
            message: "ConfiguraciÃƒÂ³n guardada exitosamente",
...
// Obtener configuraciÃƒÂ³n actual del sistema empresarial
const obtenerConfiguracion = async (req, res) => {
    try {
        ...
        // Por ahora retornamos configuraciÃƒÂ³n por defecto - aquÃƒÂ­ se obtendrÃƒÂ­a de la BD
...
const obtenerTurnos = async (req, res) => {
    try {
        ...
        // Formatear las asignaciones con informaciÃƒÂ³n completa (incluir dÃƒÂ­as del tipo de turno)
        const turnosFormateados = await Promise.all(asignaciones.map(async (asignacion) => {
            try {
                // Obtener tipo de turno con detalle de dÃƒÂ­as
    ...
    try {
        ...
        for (let reporte of reportes) {
            ...
            if (!marcacion || !marcacion.data) {
                ...
                continue; // saltar si no se encuentra la marcaciÃƒÂ³n
            }
...
const aprobarCambioMarcacion = async (req, res) => {
    try {
        ...
        console.error("Error aprobando reporte de marcaciÃƒÂ³n:", error);
...
const rechazarCambioMarcacion = async (req, res) => {
    try {
        ...
        console.error("Error rechazando reporte de marcaciÃƒÂ³n:", error);
        res.status(500).json({ success: false, message: "Error interno del servidor" });
    }
};
...
const obtenerConfiguracionTolerancias = async (req, res) => {
    try {
        ...
        console.error("Error obteniendo configuraciÃƒÂ³n de tolerancias:", error);
        res.status(500).json({ success: false, message: "Error interno del servidor" });
    }
}

// Obtener turnos especÃƒÂ­ficos de un trabajador por ID
const obtenerTurnosTrabajador = async (req, res) => {
    try {
        ...
        // Enriquecer cada turno con el detalle de dÃƒÂ­as del tipo de turno (si estÃƒÂ¡ disponible)
        ...
        console.error('Ã¢ÂÅ’ Error obteniendo turnos del trabajador:', error);
        res.status(500).json({
            success: false,
...
// Obtener marcaciones especÃƒÂ­ficas de un trabajador por ID
const obtenerMarcacionesTrabajador = async (req, res) => {
    try {
        const { id } = req.params; // ID del trabajador
        const { limite = 10 } = req.query; // LÃƒÂ­mite de marcaciones a obtener
        const USR_PETICION = req.user; // usuario que genera la consulta
        ...
        // Obtener marcaciones del trabajador usando la funciÃƒÂ³n correcta
        ...
        // Aplicar lÃƒÂ­mite manualmente ya que la funciÃƒÂ³n no lo tiene
        ...
        console.error('Ã¢ÂÅ’ Error obteniendo marcaciones del trabajador:', error);
        res.status(500).json({
            success: false,
...
const actualizarHorasLaborales = async (req, res) => {
    try {
        ...
        // Validar que las horas laborales sean vÃƒÂ¡lidas
        ...
        // Registrar auditorÃƒÂ­a (comentado temporalmente)
        // await AuditoriaModel.registrarAccion(
        //     USR_PETICION.id,
        //     'UPDATE',
        //     'usuario_empresa',
        //     trabajadorEmpresa.id,
        //     `ActualizaciÃƒÂ³n de horas laborales de ${trabajadorEmpresa.horas_laborales || 'Sin definir'} a ${horas_laborales} horas`,
        //     req.ip,
        //     req.get('User-Agent')
        // );
        ...
        console.error('Ã¢ÂÅ’ Error actualizando horas laborales:', error);
        res.status(500).json({
            success: false,
    ...
    try {
        ...
        if (req.user && req.user.id) {
            try {
                ...
                console.error('Ã¢Å¡Â Ã¯Â¸Â Error al registrar eliminaciÃƒÂ³n de tipo de turno en auditorÃƒÂ­a:', auditError);
            }
        }
        res.status(200).json({
            success: true,
...
const crearTipoTurno = async (req, res) => {
    try {
        ...
        // Validar que se proporcionen dÃƒÂ­as de trabajo
        if (!tipoTurnoData.dias || !Array.isArray(tipoTurnoData.dias) || tipoTurnoData.dias.length === 0) {
            return res.status(400).json({
                success: false,
                message: "Debe especificar al menos un dÃƒÂ­a de trabajo"
            });
        }
        ...
        if (req.user && req.user.id) {
            try {
                ...
                console.error('Error al registrar en auditorÃƒÂ­a:', auditError);
            }
        }
...
const actualizarTrabajador = async (req, res) => {
    try {
        ...
        // Obtener datos anteriores del trabajador para auditorÃƒÂ­a
        ...
        if (req.user && req.user.id) {
            try {
                ...
                console.error('Ã¢Å¡Â Ã¯Â¸Â Error al registrar en auditorÃƒÂ­a:', auditError);
            }
        }
        ...
        console.error('Ã¢ÂÅ’ Error actualizando trabajador:', error);
        res.status(500).json({
            success: false,
...
/**
 * Para turnos nocturnos, obtener la fecha "lÃƒÂ³gica" de la jornada
 * Si es entrada nocturna antes de medianoche, la jornada corresponde al dÃƒÂ­a anterior
 * Si es salida nocturna despuÃƒÂ©s de medianoche, la jornada corresponde al dÃƒÂ­a anterior
 * 
 * IMPORTANTE: Si el dÃƒÂ­a actual NO tiene turno asignado pero hay un turno nocturno el dÃƒÂ­a anterior,
 * entonces cualquier marcaciÃƒÂ³n se agrupa como parte de ese turno anterior.
 */
function obtenerFechaLogicaJornada(fechaMarcacion, horaMarcacion, tipoMarcacion, turno, turnoAnterior = null) {
    // Caso 1: El dÃƒÂ­a de la marcaciÃƒÂ³n NO tiene turno asignado
    // pero el dÃƒÂ­a anterior SÃƒÂ tiene un turno nocturno
    if (!turno && turnoAnterior && esturnoNocturno(turnoAnterior)) {
        // La marcaciÃƒÂ³n pertenece al turno nocturno anterior
        const fechaAnterior = new Date(fechaMarcacion);
        fechaAnterior.setDate(fechaAnterior.getDate() - 1);
        return fechaAnterior.toISOString().split('T')[0];
    }

    // Caso 2: El dÃƒÂ­a de la marcaciÃƒÂ³n SÃƒÂ tiene un turno nocturno
    if (esturnoNocturno(turno)) {
        ...
        // Si es una marcaciÃƒÂ³n despuÃƒÂ©s de medianoche (hora < hora_inicio del turno)
...
function calcularDiferenciaHorasNocturna(horaEntrada, horaSalida, esNocturno) {
    ...
    // significa que la salida es al dÃƒÂ­a siguiente (despuÃƒÂ©s de medianoche)
 ...
 * Similar al reporte de asistencia pero especÃƒÂ­fico para empresas
 * Maneja correctamente turnos nocturnos agrupando entrada del dÃƒÂ­a reportado con salida del dÃƒÂ­a siguiente
...
const obtenerReporteJornadaDiariaEmpresa = async (req, res) => {
    try {
        ...
        console.time('Ã¢ÂÂ±Ã¯Â¸Â Tiempo total de reporte');
        ...
        // Obtener empresa
        console.time('Ã°Å¸â€œÅ  1. Obtener empresa');
        const empresa = await EmpresaModel.getEmpresaByRut(rutEmpresa);
        console.timeEnd('Ã°Å¸â€œÅ  1. Obtener empresa');
        ...
        // OPTIMIZACIÃƒâ€œN 1: Obtener todos los trabajadores en paralelo
        console.time('Ã°Å¸â€œÅ  2. Obtener trabajadores');
        ...
        console.timeEnd('Ã°Å¸â€œÅ  2. Obtener trabajadores');
        ...
        // OPTIMIZACIÃƒâ€œN 2: Obtener TODAS las marcaciones y horas extras en paralelo
        console.time('Ã°Å¸â€œÅ  3. Obtener marcaciones y horas extras (paralelo)');
        ...
        console.timeEnd('Ã°Å¸â€œÅ  3. Obtener marcaciones y horas extras (paralelo)');

        // OPTIMIZACIÃƒâ€œN 3: Crear mapa de marcaciones y horas extras por usuario
        ...
        // OPTIMIZACIÃƒâ€œN 4: Obtener TODOS los turnos necesarios en una sola pasada
        console.time('Ã°Å¸â€œÅ  4. Obtener todos los turnos');
        ...
        Object.values(marcacionesPorUsuario).forEach(marcaciones => {
            marcaciones.forEach(m => {
                ...
                // Agregar dÃƒÂ­a anterior para turnos nocturnos
        ...
        console.timeEnd('Ã°Å¸â€œÅ  4. Obtener todos los turnos');

        // OPTIMIZACIÃƒâ€œN 5: Procesar todo en memoria
        console.time('Ã°Å¸â€œÅ  5. Procesar datos en memoria');
        ...
        for (const trabajador of todosTrabajadores) {
            ...
            if (marcaciones.length > 0) {
                for (const marcacion of marcaciones) {
                    ...
                    // Calcular fecha lÃƒÂ³gica de agrupaciÃƒÂ³n
                    ...
                    // Agregar marcaciÃƒÂ³n con hora extra
        ...
        console.timeEnd('Ã°Å¸â€œÅ  5. Procesar datos en memoria');

        console.timeEnd('Ã°Å¸â€œÅ  5. Procesar datos en memoria');

        // Filtrar y calcular horas trabajadas
        console.time('Ã°Å¸â€œÅ  6. Filtrar y calcular');
        ...
        console.timeEnd('Ã°Å¸â€œÅ  6. Filtrar y calcular');

        // Formatear respuesta
        console.time('Ã°Å¸â€œÅ  7. Formatear respuesta');
        ...
        console.timeEnd('Ã°Å¸â€œÅ  7. Formatear respuesta');

        console.timeEnd('Ã¢ÂÂ±Ã¯Â¸Â Tiempo total de reporte');
        ...
        console.error('Ã¢ÂÅ’ Error al obtener reporte de jornada diaria:', error);
        res.status(500).json({
            success: false,
...
// FunciÃƒÂ³n para obtener datos detallados de reporte de asistencia
const obtenerReporteAsistenciaDetallado = async (req, res) => {
    try {
        ...
        if (!empresa) {
            return res.status(404).json({
                success: false,
                message: "No se encontrÃƒÂ³ empresa asociada al usuario"
            });
        }
        ...
        const trabajadoresConDatos = await Promise.all(
            trabajadores.map(async (trabajador) => {
                try {
                    ...
                    // Obtener marcaciones del perÃƒÂ­odo
                    ...
                    // Calcular horas trabajadas en el perÃƒÂ­odo
                    ...
                    console.error(`Ã¢ÂÅ’ Error procesando trabajador ${trabajador.usuario_nombre}:`, error);
                    return {
                        id: trabajador.id,
        ...
        console.error('Ã¢ÂÅ’ Error obteniendo reporte detallado:', error);
        res.status(500).json({
            success: false,
...
// FunciÃƒÂ³n auxiliar para calcular estadÃƒÂ­sticas de asistencia
...
// FunciÃƒÂ³n auxiliar para calcular dÃƒÂ­as laborables
function calcularDiasLaborables(fechaInicio, fechaFin) {
    ...
    for (let d = new Date(inicio); d <= fin; d.setDate(d.getDate() + 1)) {
        ...
        if (diaSemana !== 0 && diaSemana !== 6) { // No domingo ni sÃƒÂ¡bado
            diasLaborables++;
        }
    }
...
// FunciÃƒÂ³n auxiliar para obtener informaciÃƒÂ³n de turnos
async function obtenerInformacionTurnos(turnosAsignados) {
    ...
    return await Promise.all(
        turnosAsignados.map(async (turno) => {
            try {
                // use the helper that returns the tipo turno along with its 'dias' (detalle de dÃƒÂ­as)
                const tipoTurno = await TipoTurnosModel.getByIdWithDias(turno.tipo_turno_id);

                // map dias (detalle) a un array simple de nombres de dÃƒÂ­a si existe
...
// FunciÃƒÂ³n auxiliar para calcular horas de un turno
...
async function obtenerSolicitudesUsuarios(req, res) {
    try {
        ...
        // usando el mÃƒÂ©todo obtenerPendientesPorEmpresa del modelo
...
async function aprobarSolicitud(req, res) {
    try {
        const id = parseInt(req.params.id); // Convertir a nÃƒÂºmero
        ...
        if (solicitud.subtipo === 'permiso_con_goce' || solicitud.subtipo === 'permiso_sin_goce' || solicitud.subtipo === 'uso_feriado') {
            // Llamar a la funciÃƒÂ³n para justificar ausencias
        ...
        // Enviar correo de aprobaciÃƒÂ³n
        try {
            console.log('Ã°Å¸â€œÂ§ Enviando correo de aprobaciÃƒÂ³n de solicitud...');
            await NotificacionService.enviarNotificacionAprobacionSolicitud(solicitud, user, observaciones);
            console.log('Ã¢Å“â€¦ Correo de aprobaciÃƒÂ³n enviado exitosamente');
        } catch (emailError) {
            console.error('Ã¢Å¡Â Ã¯Â¸Â Error enviando correo de aprobaciÃƒÂ³n:', emailError);
            // No bloqueamos la respuesta si falla el envÃƒÂ­o de correo
        }
...
async function rechazarSolicitud(req, res) {
    try {
        ...
        try {
            console.log('Ã°Å¸â€œÂ§ Enviando correo de rechazo de solicitud...');
            ...
            console.log('Ã¢Å“â€¦ Correo de rechazo enviado exitosamente');
        } catch (emailError) {
            console.error('Ã¢Å¡Â Ã¯Â¸Â Error enviando correo de rechazo:', emailError);
            // No bloqueamos la respuesta si falla el envÃƒÂ­o de correo
        }
