...
import ZKUsuarioSincronizacionModel from '../model/zk-usuario-sincronizacion.model.js';
...
/**
 * Obtener estado de un dispositivo especÃƒÂ­fico
 */
export const getDeviceStatus = async (req, res) => {
    try {
        ...
        // TambiÃƒÂ©n intentar obtener info de la BD
...
export const unregisterDevice = async (req, res) => {
    try {
        ...
        if (device) {
            ...
            // Si no estÃƒÂ¡ en BD pero estÃƒÂ¡ en memoria (casos raros)
        ...
        if (device && device.protocolo === 'ADMS') {
            ...
            switch (action) {
                ...
                case 'CREATE_USER':
                    ...
                    break;
                // ... aÃƒÂ±adir mÃƒÂ¡s mapeos segÃƒÂºn sea necesario
            ...
            // Esperar el resultado si es un comando sÃƒÂ­ncrono (opcional, pero mejora la experiencia)
            ...
            return res.status(200).json({
                ...
                message: waitResult.success ? 'Comando ejecutado con ÃƒÂ©xito' : (waitResult.message || 'El dispositivo recibiÃƒÂ³ el comando pero no confirmÃƒÂ³ ejecuciÃƒÂ³n'),
                data: waitResult
            });
        } else {
...
export const getUsers = async (req, res) => {
    try {
        const { serial } = req.params;
        const { forceSync } = req.query; // ParÃƒÂ¡metro opcional para forzar sincronizaciÃƒÂ³n
        ...
        // FunciÃƒÂ³n auxiliar para enriquecer usuarios con validaciÃƒÂ³n de sistema y datos completos
        ...
        if (device && device.protocolo === 'ADMS') {
            ...
            console.log(`[ADMS] Solicitando sincronizaciÃƒÂ³n de usuarios para ${serial}...`);
            const cmdId = ADMSService.queueCommand(serial, 'DATA QUERY USERINFO');

            // Esperar confirmaciÃƒÂ³n del comando
            const result = await ADMSService.waitForCommand(cmdId, 10000);

            if (result.success) {
                // El comando se ejecutÃƒÂ³, esperar un poco por los datos
                ...
                if (updatedCache && updatedCache.size > 0) {
                    ...
                    console.log(`[ADMS] Ã¢Å“â€œ ${users.length} usuarios sincronizados correctamente desde dispositivo`);
                } else {
                    return res.status(200).json({
                        success: true,
                        message: 'SincronizaciÃƒÂ³n iniciada. Los datos pueden tardar en llegar. Intente nuevamente en unos segundos.',
                        data: [],
                        syncing: true
                    });
                }
                ...
                console.log(`[ADMS] Dispositivo no respondiÃƒÂ³. Consultando BD y cache para ${serial}...`);
                ...
                // Agregar usuarios de BD primero (tienen mÃƒÂ¡s info)
                dbUsers.forEach(user => {
                    usersMap.set(user.user_id, user);
                });
                
                // Agregar usuarios del cache que no estÃƒÂ©n en BD
                if (fallbackCache && fallbackCache.size > 0) {
                    ...
                    for (const user of cacheArray) {
                        if (!usersMap.has(user.user_id)) {
                            // Usuario del cache no estÃƒÂ¡ en sistema
                ...
                if (combinedUsers.length > 0) {
                    ...
                    return res.status(200).json({
                        success: true,
                        message: 'Dispositivo no disponible. Mostrando ÃƒÂºltima sincronizaciÃƒÂ³n guardada.',
                        data: combinedUsers,
                ...
                return res.status(504).json({
                    success: false,
                    message: 'El dispositivo no respondiÃƒÂ³ y no hay datos disponibles.'
                });
            }
        } else {
        ...
        // Enriquecer usuarios con validaciÃƒÂ³n de sistema
...
export const syncTime = async (req, res) => {
    try {
        ...
        if (device && device.protocolo === 'ADMS') {
            ...
            return res.status(200).json({
                success: waitResult.success,
                message: waitResult.success ? 'Zona horaria y tiempo sincronizados con ÃƒÂ©xito' : (waitResult.message || 'El dispositivo no confirmÃƒÂ³ la sincronizaciÃƒÂ³n'),
                data: waitResult
            });
        } else {
...
export const getAttendance = async (req, res) => {
    try {
        ...
        let attendance;
        if (device && device.protocolo === 'ADMS') {
            // En ADMS la asistencia suele subir automÃƒÂ¡ticamente via cdata
            // Si queremos pedir un rango especÃƒÂ­fico:
...
export const getDeviceInfo = async (req, res) => {
    try {
        ...
        if (device && device.protocolo === 'ADMS') {
            ...
            return res.status(202).json({
                success: true,
                message: 'Comando de informaciÃƒÂ³n encolado para ADMS'
            });
        } else {
        ...
        console.error('Error obteniendo informaciÃƒÂ³n del dispositivo:', error);
        res.status(500).json({
            success: false,
            message: 'Error al obtener informaciÃƒÂ³n',
            error: error.message
        });
    }
};
...
export const restartDevice = async (req, res) => {
    try {
        ...
        if (device && device.protocolo === 'ADMS') {
            ...
            return res.status(200).json({
                success: waitResult.success,
                message: waitResult.success ? 'Dispositivo reiniciado con ÃƒÂ©xito' : (waitResult.message || 'El dispositivo no confirmÃƒÂ³ el reinicio'),
                data: waitResult
            });
        } else {
...
export const deleteUser = async (req, res) => {
    try {
        ...
        if (device && device.protocolo === 'ADMS') {
            ...
            console.log(`[ADMS] Esperando resultado de eliminaciÃƒÂ³n de usuario ${user_id} en ${serial}...`);
            const waitResult = await ADMSService.waitForCommand(cmdId, 15000);

            if (waitResult.success) {
                // Eliminar de BD tambiÃƒÂ©n
            ...
            return res.status(200).json({
                success: waitResult.success,
                message: waitResult.success ? 'Usuario eliminado con ÃƒÂ©xito' : (waitResult.message || 'El dispositivo no confirmÃƒÂ³ la eliminaciÃƒÂ³n'),
                data: waitResult
            });
        } else {
...
export const clearLogs = async (req, res) => {
    try {
        ...
        if (device && device.protocolo === 'ADMS') {
            ...
            return res.status(200).json({
                success: waitResult.success,
                message: waitResult.success ? 'Logs limpiados con ÃƒÂ©xito' : (waitResult.message || 'El dispositivo no confirmÃƒÂ³ la limpieza de logs'),
                data: waitResult
            });
        } else {
