...
/**
 * Controlador para gestiÃ³n de roles del sistema
 */
class RolesController {
    ...
    static async getAllRoles(req, res) {
        try {
            ...
            console.error('âŒ Error al obtener roles:', error);
            res.status(500).json({
                success: false,
    ...
    static async getUserRoles(req, res) {
        try {
            ...
            console.error('âŒ Error al obtener roles del usuario:', error);
            res.status(500).json({
                success: false,
    ...
    static async assignRole(req, res) {
        try {
            ...
            if (!usuarioEmpresa) {
                return res.status(404).json({
                    success: false,
                    message: 'RelaciÃ³n usuario-empresa no encontrada'
                });
            }
            ...
            console.error('âŒ Error al asignar rol:', error);
            res.status(500).json({
                success: false,
    ...
    static async revokeRole(req, res) {
        try {
            ...
            if (!revoked) {
                return res.status(404).json({
                    success: false,
                    message: 'AsignaciÃ³n de rol no encontrada'
                });
            }
            ...
            console.error('âŒ Error al revocar rol:', error);
            res.status(500).json({
                success: false,
    ...
    static async getUsersByRole(req, res) {
        try {
            ...
            console.error('âŒ Error al obtener usuarios por rol:', error);
            res.status(500).json({
                success: false,
    ...
    static async replaceUserRoles(req, res) {
        try {
            ...
            console.error('âŒ Error al reemplazar roles:', error);
            res.status(500).json({
                success: false,
    ...
    /**
     * Obtener estadÃ­sticas de roles en una empresa
    ...
    static async getRoleStats(req, res) {
        try {
            ...
            console.error('âŒ Error al obtener estadÃ­sticas de roles:', error);
            res.status(500).json({
                success: false,
