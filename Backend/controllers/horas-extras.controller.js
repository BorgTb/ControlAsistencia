...
class HorasExtrasController {
    ...
    static async aprobarHorasExtras(req, res) {
        try {
            ...
            console.log('ðŸ”µ Datos recibidos para aprobar horas extras:', req.body);
            console.log('â° Hora inicio (fin del turno):', hora_inicio);
            console.log('â° Hora fin (salida real):', hora_fin);
            ...
            console.log('ðŸ”µ Trabajador encontrado para horas extras:', trabajador);
            if (!trabajador) {
            ...
            // ðŸ” VERIFICAR SI YA EXISTE UNA HORA EXTRA PENDIENTE PARA ESTA MARCACIÃ“N
            ...
            if (marcacion_id) {
                ...
                console.log('ðŸ” Buscando hora extra existente por marcacion_id:', marcacion_id, horaExtraExistente);
            } else {
                ...
                // Filtrar las que coincidan con el horario y estÃ©n pendientes
                horaExtraExistente = horasExtrasExistentes.find(he =>
                    he.estado === 'PENDIENTE' &&
                    he.hora_inicio === hora_inicio &&
                    he.hora_fin === hora_fin
                );
                console.log('ðŸ” Buscando hora extra existente por fecha/horario:', horaExtraExistente);
            }
            ...
            if (horaExtraExistente && horaExtraExistente.estado === 'PENDIENTE') {
                // âœ… ACTUALIZAR LA HORA EXTRA EXISTENTE DE PENDIENTE A APROBADA
                console.log('ðŸ”„ Actualizando hora extra existente de PENDIENTE a APROBADA:', horaExtraExistente.id);
                ...
                // âœ… CREAR NUEVA HORA EXTRA O VALIDAR SOLAPAMIENTOS
                ...
                //       Las horas extras se calculan automÃ¡ticamente por la BD como: hora_fin - hora_inicio

                // Obtener la preferencia de compensaciÃ³n activa del trabajador
                console.log('ðŸ” Obteniendo preferencia de compensaciÃ³n del trabajador ID:', trabajador);
                ...
                const horaExtraData = {
                    ...
                    hora_fin,    // Hora real de salida (hasta cuando trabajÃ³)
                    estado: 'APROBADA',
                ...
                console.log('ðŸ’¾ Creando nueva hora extra:', horaExtraData);
            ...
            return res.status(accion === 'aprobar_existente' ? 200 : 201).json({
                ...
                message: mensaje,
                data: horaExtra,
                accion: accion // Incluir la acciÃ³n realizada en la respuesta
            });

        } catch (error) {
            console.error('âŒ Error al aprobar horas extras:', error);
            return res.status(500).json({
                success: false,
    ...
    /**
     * Aprobar una hora extra especÃ­fica que estÃ¡ PENDIENTE por su ID
     */
    static async aprobarHoraExtraPendiente(req, res) {
        try {
            ...
            console.log('ðŸ”µ Aprobando hora extra pendiente por ID:', id);
            ...
            if (horaExtraExistente.estado !== 'PENDIENTE') {
                return res.status(400).json({
                    success: false,
                    message: `La hora extra estÃ¡ en estado ${horaExtraExistente.estado}. Solo se pueden aprobar horas extras en estado PENDIENTE.`
                });
            }
            ...
            console.error('âŒ Error al aprobar hora extra pendiente:', error);
            return res.status(500).json({
                success: false,
    ...
    static async obtenerHorasExtrasPorEmpresa(req, res) {
        try {
            ...
            console.error('âŒ Error al obtener horas extras:', error);
            return res.status(500).json({
                success: false,
    ...
    static async obtenerHorasExtrasPorTrabajador(req, res) {
        try {
            ...
            console.error('âŒ Error al obtener horas extras del trabajador:', error);
            return res.status(500).json({
                success: false,
    ...
    /**
     * Obtener resumen de horas extras por empresa y perÃ­odo
     */
    static async obtenerResumenHorasExtras(req, res) {
        try {
            ...
            console.error('âŒ Error al obtener resumen de horas extras:', error);
            return res.status(500).json({
                success: false,
    ...
    /**
     * Rechazar una hora extra especÃ­fica que estÃ¡ PENDIENTE por su ID
     */
    static async rechazarHoraExtraPendiente(req, res) {
        try {
            ...
            console.log('ðŸ”´ Rechazando hora extra pendiente por ID:', id);
            ...
            if (horaExtraExistente.estado !== 'PENDIENTE') {
                return res.status(400).json({
                    success: false,
                    message: `La hora extra estÃ¡ en estado ${horaExtraExistente.estado}. Solo se pueden rechazar horas extras en estado PENDIENTE.`
                });
            }
            ...
            console.error('âŒ Error al rechazar hora extra pendiente:', error);
            return res.status(500).json({
                success: false,
    ...
    static async obtenerHorasExtrasPendientes(req, res) {
        try {
            ...
            console.error('âŒ Error al obtener horas extras pendientes:', error);
            return res.status(500).json({
                success: false,
