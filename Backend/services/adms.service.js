...
class ADMSService {
     ...
     * @param {string} sn - NÃºmero de serie del dispositivo
     */
     ...
     * @param {string} sn - NÃºmero de serie
     ...
     * @param {string} returnValue - CÃ³digo de retorno del dispositivo
     */
    ...
    async waitForCommand(cmdId, timeout = 30000) {
        return new Promise((resolve) => {
            const timer = setTimeout(() => {
                if (this.pendingResponses.has(cmdId)) {
                    this.pendingResponses.delete(cmdId);
                    resolve({ success: false, error: 'timeout', message: 'Dispositivo no respondiÃ³ a tiempo' });
                }
            }, timeout);
    ...
    /**
     * Utilidad para parsear lÃ­neas de clave-valor (formato ADMS)
     * e.g. PIN=1\tName=Alex
     */
    ...
    /**
     * Genera la respuesta de configuraciÃ³n para el handshake (cdata GET)
     */
    getHandshakeResponse() {
        const now = DateTime.now().setZone('America/Santiago');
        const formatted = now.toFormat('yyyy-MM-dd HH:mm:ss');

        // El Stamp debe ser un nÃºmero Ãºnico (usamos segundos actuales)
        const stamp = Math.floor(now.toMillis() / 1000);

        // CRÃTICO: El formato debe ser EXACTO segÃºn especificaciÃ³n ZKTeco
        // Cada parÃ¡metro en una lÃ­nea separada con \r\n
        const response = [
            'OK',
            `Stamp=${stamp}`,
            `OpStamp=${stamp}`,
    ...
    /**
     * Comandos estÃ¡ndar del protocolo ADMS
     */
    get commands() {
        return {
            syncTime: () => {
