...
import UsuarioEmpresaModel from './UsuarioEmpresaModel.js';

class ZKUsuarioSincronizacionModel {

    /**
     * Crear o actualizar registro de sincronización
     * @param {Object} sincronizacionData - Datos de la sincronización
    ...
    /**
     * Obtener estado de sincronización de un usuario en un dispositivo
     * @param {number} usuarioEmpresaId - ID del usuario empresa
     * @param {number} dispositivoId - ID del dispositivo
     * @returns {Object|null} Estado de sincronización
     */
    ...
    /**
     * Obtener estadísticas de sincronización por dispositivo
     * @param {number} dispositivoId - ID del dispositivo
    ...
    static async saveUsuarioDispositivo(dispositivoId, userId, userData) {
        ...
        try {
            ...
            if (users.length > 0) {
                // Usuario existe en el sistema, crear/actualizar sincronización
                const syncQuery = `
                    INSERT INTO zk_usuario_sincronizacion (
                        usuario_empresa_id,
                        dispositivo_id,
    ...
    static async getUsuariosConDatos(dispositivoId) {
        ...
        try {
            ...
            return rows.map(row => {
                // El user_id debe ser el mismo que usa el dispositivo (generalmente usuario_id numérico)
                const userId = row.usuario_id.toString();
                console.log(`[BD] Usuario: ${row.usuario_nombre} - ID: ${userId} - RUT: ${row.usuario_rut}`);
                
                return {
                    user_id: userId, // Usar ID numérico como en el dispositivo
    ...
    /**
     * Eliminar sincronización por user_id del dispositivo
     * @param {number} dispositivoId - ID del dispositivo
     * @param {string} userId - ID del usuario en el dispositivo
     * @returns {Object} Resultado de la operación
     */
