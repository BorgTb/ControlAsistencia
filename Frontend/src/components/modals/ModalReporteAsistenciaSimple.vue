...
const cargarDatosReporte = async () => {
  ...
  try {
    ...
    // Procesar cada trabajador para obtener sus horas trabajadas
    const trabajadoresConHoras = await Promise.all(
      dataTrabajadores.data.map(async (trabajador) => {
        try {
          // Obtener horas trabajadas usando el mismo método que GestionTrabajadores
          console.log(`🧮 Calculando horas trabajadas para trabajador ID: ${trabajador.id} (${trabajador.usuario_nombre})`);
          const resultado = await EmpresaServices.obtenerHorasSemanales(trabajador.id);
          console.log(`� Resultado horas trabajadas para ${trabajador.usuario_nombre}:`, resultado);
          
          let horasSemanales = 0;
          if (resultado && resultado.success && resultado.data) {
            horasSemanales = resultado.data.totalHorasSemanales || 0;
            console.log(`✅ Horas trabajadas esta semana: ${horasSemanales}`);
          } else {
            console.warn(`⚠️ No se pudieron obtener horas trabajadas para ${trabajador.usuario_nombre}:`, resultado);
            horasSemanales = 0;
          }

          // Aplicar el mismo formato que GestionTrabajadores
          const horasFormateadas = Math.round(horasSemanales * 100) / 100; // 2 decimales
          console.log(`📊 Horas formateadas: ${horasFormateadas}`);

          // Obtener las horas asignadas para comparación
          const horasAsignadas = trabajador.horas_laborales ? parseInt(trabajador.horas_laborales) : 45;
          const excedeHoras = parseFloat(horasSemanales) > horasAsignadas;
          
          console.log(`🎯 RESULTADO: ${trabajador.usuario_nombre} - ${horasFormateadas} hrs trabajadas (asignadas: ${horasAsignadas}, excede: ${excedeHoras})`);
          ...
          // Calcular estadísticas básicas desde las horas trabajadas
          if (horasSemanales > 0) {
            diasCumplidos = Math.min(5, Math.ceil(horasSemanales / 8)); // Días estimados
          }
          ...
          console.log(`🔥 ANTES DE RETURN - ${trabajador.usuario_nombre}: horas_trabajadas será: ${horasFormateadas.toString()}`);
          ...
          return {
            ...
            iniciales: iniciales || '--',
            horas_trabajadas: horasFormateadas.toString(), // Mostrar horas trabajadas reales
            horas_asignadas: horasAsignadas, // Para cálculos internos
            excede_horas: excedeHoras, // Basado en comparación real
            dias_cumplidos: diasCumplidos,
            llegadas_tarde: llegadasTarde,
            ausencias: ausencias,
            promedio_diario: (diasCumplidos > 0 ? (horasFormateadas / diasCumplidos).toFixed(1) : '0.0'),
            turnos: [
              {
                dia: 'Lunes a Viernes',
                horario: '09:00 - 17:00',
                tipo: 'Día',
                horas_programadas: (diasCumplidos > 0 ? (horasFormateadas / diasCumplidos).toFixed(1) : '0.0'),
